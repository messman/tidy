...
/***/ "./node_modules/css-loader/index.js!./node_modules/sass-loader/lib/loader.js!./src/views/settings/settings.scss":
/***/ (function(module, exports, __webpack_require__) {
...
/***/ "./node_modules/css-loader/index.js!./node_modules/sass-loader/lib/loader.js!./src/views/tide/title/title.scss":
/***/ (function(module, exports, __webpack_require__) {
...
var DEFINE = exports.DEFINE = {
    BUILD: {
        IS_PRODUCTION: false,
        TIME: "Fri Jul 20 2018 14:27:20 GMT-0400 (EDT)"
    },
    DEBUG: {
...
function newDatePlusHours(addHours, date) {
    var newDate = null;
    if (date) newDate = new Date(date.getTime());else newDate = new Date();
...
function formatDateForRequest(d) {
    //yyyyMMdd HH:mm
    var twosNum = [d.getMonth() + 1, d.getDate(), d.getHours(), d.getMinutes()];
    var twos = twosNum.map(function (num) {
        return num.toString().padStart(2, "0");
    });
    return "" + d.getFullYear() + twos[0] + twos[1] + " " + twos[2] + ":" + twos[3];
}
function makeRequest(options) {
    var url = createRequest(options);
    return fetch(url).then(function (response) {
        if (response.ok) return response.json();else return Promise.reject(response);
    });
}
function getWaterLevelData(date, predictionHoursRadius) {
    ...
    if (!_define.DEFINE.BUILD.IS_PRODUCTION && _define.DEFINE.DEBUG.LOCAL_REQUEST_DATA) {
        var fakeResponse = {
            ...
            predictions: [{ time: parseTimeFromResponse("2018-07-18 10:07"), val: -5.491, isHigh: false }, { time: parseTimeFromResponse("2018-07-18 16:33"), val: 5.273, isHigh: true }, { time: parseTimeFromResponse("2018-07-18 22:37"), val: -4.809, isHigh: false }, { time: parseTimeFromResponse("2018-07-19 04:56"), val: 5.013, isHigh: true }, { time: parseTimeFromResponse("2018-07-19 11:05"), val: -4.995, isHigh: false }, { time: parseTimeFromResponse("2018-07-19 17:30"), val: 5.095, isHigh: true }, { time: parseTimeFromResponse("2018-07-19 23:41"), val: -4.536, isHigh: false }, { time: parseTimeFromResponse("2018-07-20 05:57"), val: 4.432, isHigh: true }],
            current: {
        ...
        var fakeLevel = parseWaterLevel(fakeResponse);
    ...
    var predictionPromise = makeRequest(predictionsOpts).then(function (json) {
        // Create the processed data
        response.predictions = json.predictions.map(function (p) {
            return {
    ...
    return Promise.all([predictionPromise, currentLevelPromise]).then(function () {
        // response object should be ready
        var waterLevel = parseWaterLevel(response);
        if (!_define.DEFINE.BUILD.IS_PRODUCTION) console.log(waterLevel);
        return waterLevel;
    });
}
var default_predictionHoursRadius = 24;
...
function parseWaterLevel(response) {
    var waterLevel = {
    ...
    if (!response.errors) {
        ...
        var indexOfClosestBefore = 0;
        var currentTime = response.current.time.getTime();
        var minTimeDiff = Infinity;
        for (var i = 0; i < response.predictions.length; i++) {
            var prediction = response.predictions[i];
            var diff = currentTime - prediction.time.getTime();
            if (diff < 0) break;
            if (diff < minTimeDiff) {
...
var _settings = __webpack_require__("./src/views/settings/settings.tsx");
...
var App = function (_React$Component) {
    ...
    function App(props) {
        ...
        _this.beginWaterLevelRequest = function () {
            _this.setState({ waterLevelIsRequesting: true });
        };
        _this.endWaterLevelRequest = function (waterLevel) {
            _this.setState({
...
/***/ "./src/views/settings/settings.scss":
/***/ (function(module, exports, __webpack_require__) {


var content = __webpack_require__("./node_modules/css-loader/index.js!./node_modules/sass-loader/lib/loader.js!./src/views/settings/settings.scss");
...
__webpack_require__("./src/views/settings/settings.scss");
...
var _noaa = __webpack_require__("./src/services/noaa.tsx");
...
var Tide = exports.Tide = function (_React$Component) {
    ...
    _createClass(Tide, [{
        ...
        value: function componentDidMount() {
            var waterLevel = this.props.waterLevel;

            if (!waterLevel.data && !waterLevel.isRequesting) {
                (0, _noaa.getWaterLevelData)().then(function (data) {
        ...
        value: function componentWillUnmount() {
            ...
            if (!waterLevel.data && waterLevel.isRequesting) this.props.waterLevel.onRequestEnd(null);
        }
    }, {
        key: "render",
        value: function render() {
            ...
            if (!waterLevel.data && !waterLevel.isRequesting || waterLevel.data && waterLevel.data.errors) {
                ...
                return React.createElement("div", { className: "tide" }, React.createElement(_title.Title, { waterLevel: data }), React.createElement(_wave.Wave, { waterLevel: data }), React.createElement("button", { className: "low-button share", onClick: this.clickShare }, React.createElement("svg", { version: "1.1", xmlns: "http://www.w3.org/2000/svg", width: "23", height: "30", viewBox: "0 0 23 30" }, React.createElement("path", { d: "M19.688 3.75h-4.688c0-2.068-1.682-3.75-3.75-3.75s-3.75 1.682-3.75 3.75h-4.688c-1.553 0-2.813 1.259-2.813 2.813v20.625c0 1.553 1.259 2.813 2.813 2.813h16.875c1.553 0 2.813-1.259 2.813-2.813v-20.625c0-1.553-1.259-2.813-2.813-2.813zM19.336 27.188h-16.172c-0.194 0-0.352-0.157-0.352-0.352v0-19.922c0-0.194 0.157-0.352 0.352-0.352v0h2.461v2.109c0 0.388 0.315 0.703 0.703 0.703h9.844c0.388 0 0.703-0.315 0.703-0.703v-2.109h2.461c0.194 0 0.352 0.157 0.352 0.352v0 19.922c0 0.194-0.157 0.352-0.352 0.352v0zM11.25 2.344c0.777 0 1.406 0.63 1.406 1.406s-0.63 1.406-1.406 1.406-1.406-0.63-1.406-1.406 0.63-1.406 1.406-1.406z" })), React.createElement("span", { className: "low-button-title" }, "Share")), React.createElement("button", { className: "low-button refresh", onClick: this.clickRefresh }, React.createElement("span", { className: "low-button-title" }, "Refresh"), React.createElement("svg", { version: "1.1", xmlns: "http://www.w3.org/2000/svg", width: "30", height: "30", viewBox: "0 0 30 30" }, React.createElement("path", { d: "M21.722 7.809c-1.832-1.715-4.209-2.655-6.731-2.653-4.539 0.004-8.457 3.116-9.539 7.433-0.079 0.314-0.359 0.536-0.683 0.536h-3.358c-0.439 0-0.773-0.399-0.692-0.831 1.268-6.732 7.179-11.826 14.28-11.826 3.893 0 7.429 1.531 10.038 4.025l2.093-2.093c0.886-0.886 2.401-0.258 2.401 0.994v7.855c0 0.777-0.63 1.406-1.406 1.406h-7.855c-1.253 0-1.88-1.515-0.994-2.401l2.446-2.446zM1.875 17.344h7.855c1.253 0 1.88 1.515 0.994 2.401l-2.446 2.446c1.832 1.715 4.209 2.655 6.731 2.653 4.536-0.004 8.456-3.114 9.538-7.433 0.079-0.314 0.359-0.536 0.683-0.536h3.358c0.439 0 0.773 0.399 0.692 0.831-1.268 6.732-7.179 11.826-14.28 11.826-3.893 0-7.429-1.531-10.038-4.025l-2.093 2.093c-0.886 0.886-2.401 0.258-2.401-0.994v-7.855c0-0.777 0.63-1.406 1.406-1.406z" }))));
            }
        }
    }]);
...
/***/ "./src/views/tide/title/title.scss":
/***/ (function(module, exports, __webpack_require__) {


var content = __webpack_require__("./node_modules/css-loader/index.js!./node_modules/sass-loader/lib/loader.js!./src/views/tide/title/title.scss");
...
/***/ "./src/views/tide/title/title.tsx":
/***/ (function(module, exports, __webpack_require__) {
...
//# sourceMappingURL=index.bb3b3919a9.js.map